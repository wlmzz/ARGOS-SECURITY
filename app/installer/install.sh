...
_pip_install owasp-nettacker
_pip_install power-pwn
_pip_install paho-mqtt
_pip_install nebula-ai
...
# ─── 6. Offensive payloads + stress tools + red team tools ───────────────────
step "Step 6/9 — Offline tool cache (payloads + stress + red team)"
...
# Stress testing tools (auto-cloned on first use, but pre-install here for speed)
mkdir -p /opt/argos/stress
_clone_tool() {
    ...
    if [ -d "$dest" ]; then
        ...
        git clone --depth=1 -q "https://github.com/$repo.git" "$dest" 2>/dev/null \
            ...
            || warn "$name clone failed — will retry on first use"
        [ -f "$dest/requirements.txt" ] && \
            pip3 install -q --break-system-packages -r "$dest/requirements.txt" 2>/dev/null
    fi
}
_clone_tool "MatrixTM/MHDDoS"      "/opt/argos/stress/MHDDoS"   "MHDDoS (DoS resilience testing)"
_clone_tool "LimerBoy/Impulse"     "/opt/argos/stress/Impulse"   "Impulse (DoS resilience testing)"
_clone_tool "palahsu/DDoS-Ripper"  "/opt/argos/stress/DRipper"   "DDoS-Ripper (DoS resilience testing)"
_clone_tool "jasonxtn/Kraken"      "/opt/argos/stress/Kraken"    "Kraken (HTTP stress testing)"
_clone_tool "TryCatchHCF/Cloakify" "/opt/argos/tools/cloakify"   "Cloakify (DLP evasion testing)"
_clone_tool "elder-plinius/L1B3RT4S" "/opt/argos/tools/L1B3RT4S" "L1B3RT4S (LLM red team prompts)"
_clone_tool "qsecure-labs/overlord" "/opt/argos/tools/overlord"   "overlord (red team infra)"
...
add_env_comment "PENTEST_SWARM_URL"        "Pentest-Swarm-AI URL (e.g. http://localhost:8080)"
add_env_comment "PENTESTSWARM_ORCHESTRATOR_API_KEY" "Pentest-Swarm-AI LLM API key (Claude/Anthropic)"
add_env_comment "POWERPWN_AUTH_TOKEN"      "power-pwn Azure AD bearer token (M365 pentesting)"
add_env_comment "PENTEST_COPILOT_USER"     "Pentest Copilot username (for auto-login)"
add_env_comment "PENTEST_COPILOT_PASS"     "Pentest Copilot password (for auto-login)"
add_env_comment "WPSCAN_API_TOKEN"         "WPScan API token — wpscan.com/register (free: 25 API requests/day)"
add_env_comment "NEBULA_MODEL"             "Nebula AI model name (Ollama: llama3, mistral, etc.)"
add_env_comment "SPIDERFOOT_URL"           "SpiderFoot server URL (optional, default: http://localhost:5001)"
...
command -v powerpwn  &>/dev/null && echo "  ✓ power-pwn (M365 pentest)" || echo "  ✗ power-pwn (pip install power-pwn)"
python3 -m owasp_nettacker --version &>/dev/null 2>&1 && echo "  ✓ OWASP Nettacker" || echo "  ✗ OWASP Nettacker (pip install owasp-nettacker)"
[ -d "$PAYLOADS_DIR" ] && echo "  ✓ Offensive-Payloads ($PAYLOADS_DIR)" || echo "  ✗ Offensive-Payloads (will fetch on demand)"
command -v skipfish &>/dev/null && echo "  ✓ skipfish" || echo "  ✗ skipfish (apt install skipfish)"
command -v nebula   &>/dev/null && echo "  ✓ nebula-ai (AI pentest)" || echo "  ✗ nebula (pip install nebula-ai)"
[ -d "/opt/argos/stress/MHDDoS" ]   && echo "  ✓ MHDDoS (DoS resilience)" || echo "  ✗ MHDDoS (auto-cloned on first use)"
[ -d "/opt/argos/stress/Impulse" ]  && echo "  ✓ Impulse (DoS resilience)" || echo "  ✗ Impulse (auto-cloned on first use)"
[ -d "/opt/argos/stress/Kraken" ]   && echo "  ✓ Kraken (HTTP stress)" || echo "  ✗ Kraken (auto-cloned on first use)"
[ -d "/opt/argos/tools/cloakify" ]  && echo "  ✓ Cloakify (DLP evasion test)" || echo "  ✗ Cloakify (auto-cloned on first use)"
echo ""
echo "  Services:"
